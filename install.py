...
team_id = createPage('People', 'Cilvēki')
...
createPage('Events', 'Notikumi', conf_text, conf_text)
...
def createPerson (person):
	...
	if person.get('interests_lv'):
		p.interests_lv = person.get('interests_lv')
	if person.get('interests_en'):
		p.interests_en = person.get('interests_en')
	...
	if person.get('github'):
		p.github_url = person.get('github')
	if person.get('url'):
		p.homepage_url = person['url']
